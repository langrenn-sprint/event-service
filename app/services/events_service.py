...
class EventsService:
    ...
    @classmethod
    async def create_event(cls: Any, event: Event) -> UUID | None:
        ...
        """
        # Chekc if the event with id already exists:
        existing_event = await EventsAdapter.get_event_by_id(event.id)
        if existing_event:
            msg = f"Event with id {event.id} already exists."
            raise IllegalValueError(msg) from None
        # Validate new event:
        await validate_event(event)
        # insert new event
        result = await EventsAdapter.create_event(event)
        cls.logger.debug(f"inserted event with id: {event.id}")
        if not result:
            return None
        return event.id

    @classmethod
    async def update_event(cls: Any, event_id: UUID, event: Event) -> str | None:
        """Get event function."""
        # validate:
        await validate_event(event)
        # get old document
        ...
        if event.id != old_event.id:
            ...
            raise IllegalValueError(msg) from None
        return await EventsAdapter.update_event(event_id, event)

    @classmethod
    async def delete_event(cls: Any, event_id: UUID) -> str | None:
        """Get event function."""
        # get old document
...
async def validate_event(event: Event) -> None:
    ...
    # Validate competition_format:get_competition_formats_by_name
